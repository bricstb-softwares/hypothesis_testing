...
def estimate_pds_over_same_support_from_hist(p_counts, p_edges, q_counts, q_edges):
    ...
    if len(p_edges) > len(q_edges):
        p_counts = rebin_histogam(old_bin_edges=p_edges, old_bin_count=p_counts, new_bin_edges=q_edges)
        p_edges  = q_edges.copy()
    elif len(p_edges) < len(q_edges):     
        q_counts = rebin_histogam(old_bin_edges=q_edges, old_bin_count=q_counts, new_bin_edges=p_edges)
